  ...
  return (
    <main className="p72">
      <header className="p72-nav">
        ...
        <a href="#simulador" className="p72-btn p72-btn-dark">Crear mi plan <span>↓</span></a>
      </header>

      <section className="p72-hero" id="inicio">
        <div className="p72-hero-copy">
          <div className="p72-hero-eyebrow"><span className="p72-live-dot" /> Preparación personal para emergencias reales</div>
          <h1>Primero la ruta.<br/><em>Después la mochila.</em></h1>
          <p className="p72-lead">Plan72 convierte tu ubicación en una salida posible, un protocolo claro y el equipo para sostener las primeras 72 horas.</p>
          <div className="p72-actions"><a href="#simulador" className="p72-btn p72-btn-lime">Empezar mi simulación <span>→</span></a><a href="#sistema" className="p72-btn p72-btn-light">Ver cómo funciona</a></div>
          <div className="p72-trust-row"><span><b>01</b> Ubicación real</span><span><b>02</b> Ruta adaptable</span><span><b>03</b> Equipo adecuado</span></div>
        </div>
        <div className="p72-hero-art" aria-label="Equipo Plan72 preparado para una salida de 72 horas">
          <div className="p72-art-route"><span>BARCELONA</span><i/><i/><i/><b>ZONA SEGURA</b></div>
          <span className="p72-art-label"><i className="p72-live-dot"/> Plan activo · 72 h</span>
          ...
          <div className="p72-art-card"><span>72</span><p><small>PLAN PERSONAL</small>Tu salida, lista para revisar.</p><div><small>COBERTURA</small><strong>2 personas</strong></div></div>
          <div className="p72-floating-card"><small>SIGUIENTE DECISIÓN</small><strong>Revisar punto de salida</strong><span>08 min</span></div>
        </div>
      </section>
      ...
      <LandingSimulator backpack={{ id: packs[pack].name.toLowerCase(), name: packs[pack].name, basePrice: packs[pack].price }} people={people} />

      <section className="p72-outcome">
        <div className="p72-outcome-copy"><p className="p72-kicker">Cuando terminás la simulación</p><h2>No recibís una línea. Recibís un plan que podés leer de un vistazo.</h2><p>Origen, ruta, puntos para reevaluar y el equipo que sostiene cada decisión. Todo conectado en una sola vista.</p><a href="#simulador" className="p72-text-link">Crear mi primer plan <span>→</span></a></div>
      ...
      <section className="p72-packs" id="mochilas">
        ...
        <div className="p72-config"><div><p className="p72-kicker">Tu configuración</p><h3>{packs[pack].name} · {people} {people === 1 ? "persona" : "personas"}</h3></div><div className="p72-stepper"><button aria-label="Quitar una persona" onClick={() => setPeople(Math.max(1, people - 1))}>−</button><span>{people}</span><button aria-label="Agregar una persona" onClick={() => setPeople(Math.min(8, people + 1))}>+</button></div><div className="p72-total"><small>Estimación</small><strong>€{total}</strong></div><a href="#simulador" className="p72-btn p72-btn-dark">Crear este plan →</a></div>
      </section>

      <section className="p72-final"><p className="p72-kicker">Tu primera decisión puede ser ahora</p><h2>En una emergencia, el plan no debería empezar con una compra.</h2><p>Debería empezar sabiendo desde dónde salís.</p><a href="#simulador" className="p72-btn p72-btn-cream">Ubicarme y empezar →</a></section>
