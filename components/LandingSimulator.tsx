...
export default function LandingSimulator({ backpack, people }: { backpack: { id: string; name: string; basePrice: number }; people: number }) {
  ...
  return <section className="p72-simulator" id="simulador">
    ...
    <div className="p72-sim-shell">
      ...
      <div className={`p72-map-stage ${mapTransition ? "is-zooming" : ""}`}>
        {selectedCity && (stage >= 3 || mapTransition)
          ? <>
              <CityMap city={city} center={{ lat: selectedCity.lat, lng: selectedCity.lng }} boundingBox={selectedCity.boundingBox} boundary={selectedCity.geojson} address={selectedAddress ? { label: address, lat: selectedAddress.lat, lng: selectedAddress.lng } : null} />
              {mapTransition && <div className="p72-zoom-transition p72-map-handoff"><i/><i/><button onClick={finishMapTransition}>Mapa urbano listo <span>→</span></button></div>}
            </>
          : <div className="p72-globe"><Globe3D selectedCountry={countryName} /><div className="p72-globe-label"><strong>{countryName}</strong><span>EXPLORÁ EL MAPA</span></div></div>}
      </div>
    </div>
